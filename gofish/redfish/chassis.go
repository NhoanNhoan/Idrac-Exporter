...
func GetChassis(c common.Client, uri string) (*Chassis, error) {
	...
	var chassis Chassis
	err = fastjson.NewDecoder(resp.Body).Decode(&chassis)
	if err != nil {
		return nil, err
	}
...
func (chassis *Chassis) Thermal() (*Thermal, error) {
	...
	var thermal Thermal
	err = fastjson.NewDecoder(resp.Body).Decode(&thermal)
	if err != nil {
		return nil, err
	}
...
func (chassis *Chassis) Power() (*Power, error) {
	...
	var power Power
	err = fastjson.NewDecoder(resp.Body).Decode(&power)
	if err != nil {
		return nil, err
	}
