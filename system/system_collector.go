...
func (collector SystemCollector) collectVolumes(ch chan<- prometheus.Metric, storage *redfish.Storage) {
	...
	for _, volume := range volumes {
		...
		ch <- prometheus.MustNewConstMetric(config.S_storage_volume,
			...
			float64(status),
			volume.Description,
			collector.convertCapacity(float64(volume.CapacityBytes)),
			...
			fmt.Sprintf("%v", collector.associatedDriveIds(volume)),
		)
	}
}
